# builds AWS resource type constraints using SMT-LIB syntax and action encoding (offline)
    ...
    if len(resource_types) > 0:
        ...
        for resource_type in encoding_json[namespace]:
            ...
            if resource_type in encoding_json[namespace] and len(encoding_json[namespace][resource_type]) > 0:
                ...
                if lo == hi:
                    smt += ' (= (str.to_int action) {})'.format(int(lo))
                else:
...
file = open('constraints_enc_z3.json', 'w')
